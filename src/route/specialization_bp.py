...
def add():
    ...
    if not lengthOfStudy:
        return sendResponse(400, 11, {"message": "lengthOfStudy missing"}, "error")
    try:
        lengthOfStudy = int(lengthOfStudy)
    except:
        return sendResponse(400, 11, {"message": "lengthOfStudy not integer"}, "error")
    if not abbrevation:
        return sendResponse(400, 11, {"message": "abbrevation missing"}, "error")
    if len(abbrevation) > 1:
        return sendResponse(400, 11, {"message": "abbrevation too long"}, "error")
    if Specialization.query.filter_by(abbrevation = abbrevation).first():
        return sendResponse(400, 11, {"message": "abbrevation already in use"}, "error")
    if not name:
        return sendResponse(400, 11, {"message": "name missing"}, "error")
    if Specialization.query.filter_by(name = name).first():
        return sendResponse(400, 11, {"message": "name already in use"}, "error")
    ...
    db.session.commit()
    return sendResponse (201, 10, {"message": "Specialization created succesfuly"}, "succes")
...
def delete():
    ...
    if not Specialization.query.filter_by(id = idSpecialization).first():
        return sendResponse(400, 11, {"message": "Wrong idSpecialization"}, "error")
    if Class.query.filter_by(idSpecialization = idSpecialization).first():
        return sendResponse(400, 11, {"message": "Some class still uses this specialization"}, "error")
    db.session.delete(Specialization.query.filter_by(id = idSpecialization).first())
    db.session.commit()
    return sendResponse (201, 10, {"message": "Specialization deleted succesfuly"}, "succes")