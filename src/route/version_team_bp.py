...
async def add():
    ...
    if Task.query.filter_by(id = idTask).first().deadline < datetime.datetime.now():
        return send_response(400, 38060, {"message": "Cannot update version after deadline"}, "error")
    ...
    if not status:
        return send_response(400, 38070, {"message": "File already exists"}, "error")
    
    new_version = Version_Team(idTeam = idTeam, idTask = idTask, elaboration = elaboration.filename, idVersion = id)
    db.session.add(new_version)
    db.session.commit()

    return send_response(400, 38081, {"message": "Version_team created"}, "success")
...
async def change():
    ...
    if Task.query.filter_by(id = idTask).first().deadline < datetime.datetime.now():
        return send_response(400, 49040, {"message": "Cannot update version after deadline"}, "error")
    ...
    return send_response(400, 49051, {"message": "Version_team updated"}, "success")
