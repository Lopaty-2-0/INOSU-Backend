...
def login():
    ...
    return send_response(200, 6031, {"message": "Login successful", "user": {"id": user.id, "name": user.name, "surname": user.surname, "abbreviation": user.abbreviation, "role": user.role, "profilePicture": user.profilePicture, "email": user.email, "idClass": all_user_classes(user.id), "createdAt":user.createdAt}}, "success")
...
def verifyUser():
    ...
    return send_response(200, 17011, {"message":message,"logged": state, "role":role, "id":id}, "success")